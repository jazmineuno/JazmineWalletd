...
void __stdcall serviceMain(DWORD dwArgc, char **lpszArgv) {
	...
	serviceStatusHandle = RegisterServiceCtrlHandler((LPCWSTR)"PaymentGate", serviceHandler);
...
int runDaemon() {
	...
	SERVICE_TABLE_ENTRY serviceTable[]{
		{ (LPWSTR)"Payment Gate", (LPSERVICE_MAIN_FUNCTIONW)serviceMain },
		{ NULL, NULL }
	};
...
int registerService() {
	...
	for (;;) {
		if (GetModuleFileName(NULL, (LPWSTR)pathBuff, ARRAYSIZE(pathBuff)) == 0) {
		...
		scService = CreateService(scManager, (LPCWSTR)SERVICE_NAME, NULL, SERVICE_QUERY_STATUS, SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START,
			SERVICE_ERROR_NORMAL, (LPCWSTR)modulePath.c_str(), NULL, NULL, NULL, NULL, NULL);
...
int unregisterService() {
	...
	for (;;) {
		...
		scService = OpenService(scManager, (LPCWSTR)SERVICE_NAME, SERVICE_STOP | SERVICE_QUERY_STATUS | DELETE);
		if (scService == NULL) {
